...
/* -----------------------------------------------------------
   SHOWCASE MODE (scroll / tabbed) + TABS
   ----------------------------------------------------------- */
function initShowcaseControls() {
  ...
  var scrollEl = document.getElementById('showcase-scroll');
  var tabbedEl = document.getElementById('showcase-tabbed');
  function setMode(mode) {
    var tabbed = mode === 'tabbed';
    root.classList.remove('iw-mode-scroll', 'iw-mode-tabbed');
    root.classList.add(tabbed ? 'iw-mode-tabbed' : 'iw-mode-scroll');
    if (scrollEl) scrollEl.style.display = tabbed ? 'none' : 'block';
    if (tabbedEl) tabbedEl.style.display = tabbed ? 'flex' : 'none';
    modeBtns.forEach(function (b) {
      var on = b.getAttribute('data-mode-btn') === mode;
      b.style.background = on ? '#80c34a' : 'transparent';
  ...
  // tabs (within tabbed mode)
  var TAB_TITLES = ['Our Work', 'Partners & Awards', 'What our clients say'];
  var TAB_SUBS   = ['', '', 'Our clients love our work & the results.'];
  ...
  function setTab(i) {
    panels.forEach(function (p) {
      p.hidden = parseInt(p.getAttribute('data-tab-panel'), 10) !== i;
    });
    tabBtns.forEach(function (b) {
      var on = parseInt(b.getAttribute('data-tab-btn'), 10) === i;
      b.style.background = on ? '#1266b5' : 'transparent';
      b.style.color = on ? '#fff' : '#5a6b82';
    });
    if (titleEl) titleEl.textContent = TAB_TITLES[i];
    if (subEl) subEl.textContent = TAB_SUBS[i];
  }
  tabBtns.forEach(function (b) {
    b.addEventListener('click', function () { setTab(parseInt(b.getAttribute('data-tab-btn'), 10)); });
  });
  setTab(0);
}
